...
void update_next_availability_window()
{
    ...
    ESP_LOGI(orchestration_log_prefix, "Next time we are available is at %"PRIu64".", next_time);
}
...
void sdp_orchestration_parse_next_message(work_queue_item_t *queue_item)
{
    ...
    ESP_LOGI(orchestration_log_prefix, "Peer %s is available at %"PRIu64".", queue_item->peer->name, queue_item->peer->next_availability);
}
...
int sdp_orchestration_send_next_message(work_queue_item_t *queue_item)
{
    ...
    ESP_LOGI(orchestration_log_prefix, "BEFORE NEXT get_time_since_start() = %"PRIu64, get_time_since_start());
    /* TODO: Handle the 64bit loop-around after 79 days ? */
    uint64_t delta_next = next_time - get_time_since_start();
    ESP_LOGI(orchestration_log_prefix, "BEFORE NEXT delta_next = %"PRIu64, delta_next);
...
void sleep_until_peer_available(sdp_peer *peer, uint64_t margin_us)
{
    if (peer->next_availability > 0)
    {
        ...
        ESP_LOGI(orchestration_log_prefix, "Going to sleep for %"PRIu64" microseconds.", sleep_length);
...
bool ask_for_time(uint64_t ask) {
    ...
    if ((wait_time_left < ask) && (requested_time < ask - wait_time_left)) {
        ...
        if (esp_timer_get_time() + wait_time_left + ask < SDP_AWAKE_TIMEBOX_uS) {
            ...
            ESP_LOGI(orchestration_log_prefix, "Orchestrator granted an extra %"PRIu64" ms of awakeness.", ask/1000);
            return true;
        } else {
            ESP_LOGE(orchestration_log_prefix, "Orchestrator denied %"PRIu64" ms of awakeness because it would violate the timebox.", ask/1000);
            return false;      
        }
    } 
    ESP_LOGD(orchestration_log_prefix, "Orchestrator got an unnessary request for %"PRIu64" ms of awakeness.", ask/1000);
    return true;
...
void take_control()
{
    ...
    while (1) {
        ...
        ESP_LOGI(orchestration_log_prefix, "Orchestrator awaiting sleep for %"PRIu64" ms.", wait_ms);
        wait_for_sleep_started = esp_timer_get_time();
        vTaskDelay(wait_ms / portTICK_PERIOD_MS);
        if (requested_time > 0) {
            wait_time = requested_time;
            ESP_LOGI(orchestration_log_prefix, "Orchestrator extending the awake phase.");
            requested_time = 0;
        } else {
            ESP_LOGI(orchestration_log_prefix, "------------Orchestrator done waiting, going to sleep! -----------");
            break;
        }
    ...
    {
        ESP_LOGI(orchestration_log_prefix, "----------------- Calling before sleep callback ------------------");
        if (!on_before_sleep_cb())
        {
            ESP_LOGW(orchestration_log_prefix, "----------Stopped from going to sleep by callback!! -----------------");
            return;
        }
    }
...
void give_control(sdp_peer *peer)
{
    ...
    if (peer != NULL)
    {
        ...
        ESP_LOGI(orchestration_log_prefix, "Asking for orchestration..");
        int retries = 0;
        ...
        if (retries == 10)
        {
            ESP_LOGE(orchestration_log_prefix, "Haven't gotten an availability time for peer \"%s\" ! Tried %i times. Going to sleep for %i microseconds..",
        ...
        else
        {
            ...
            ESP_LOGI(orchestration_log_prefix, "Waiting for sleep..");
...
{
    orchestration_log_prefix = _log_prefix;
